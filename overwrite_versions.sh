...
if [[ ! -z "$VERSION_MAJOR" ]]; then
    ...
    sed -i -E "s/versionMajor *= *.+/versionMajor = $VERSION_MAJOR/" config.gradle
    sed -i -E "s/versionMinor *= *.+/versionMinor = $VERSION_MINOR/" config.gradle
    sed -i -E "s/versionPatch *= *.+/versionPatch = $VERSION_PATCH/" config.gradle
    sed -i -E "s/versionClassifier *= *.+/versionClassifier = \"$VERSION_CLASS\"/" config.gradle
